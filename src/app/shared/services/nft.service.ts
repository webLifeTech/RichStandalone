  ...
  async connectWallet(recieverContractId?: any): Promise<string | null> {
    if ((window as any).ethereum) {
      try {
        ...
        this.contract = new this.web3.eth.Contract(this.abi, recieverContractId); // Now contractAddress getting from CLT API recieverContractId
        return accounts[0];
      } catch (error) {
