...
export class UserCarsListingComponent {
  ...
  editInfo: any = {};
  booktabs = [
    { id: 1, title: 'All Vehicles', value: 'all_cars' },
    ...
    // { title: "All Vehicles", value: "all_cars" },
  ...
  getTableData(type: any) {
    ...
    this.mycarsService.getMyCars().subscribe((apiRes: apiResultFormat) => {
      ...
      filteredData = allCars.filter((item: any) => {
        const matchesTab = type === 'all_cars' ? item : item.user_type === tabName;
        const matchesStatus = this.searchFilter.status === 'all' ? item : (item.status === this.searchFilter.status || item.vehicle_status === this.searchFilter.status);
